...
class ProteinBertModel(nn.Module):
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False, res_id=None):
        if return_contacts:
        ...
        x = x + self.embed_positions(tokens, positions=res_id)
...
class MSATransformer(nn.Module):
    ...
    def forward(
        ...
        if res_idx is not None:
            res_idx = res_idx.view(batch_size, -1)
            if res_idx.size(-1) == seqlen - 1:
                res_idx = torch.cat([(res_idx[..., 0:1] - 1) * torch.ones_like(res_idx[..., 0:1]), res_idx], dim=-1)
            assert res_idx.size() == (batch_size, seqlen), f'res_idx shape:{res_idx.size()}'
            res_idx = res_idx[:, None, :].repeat((1, tokens.size(1), 1, 1))


        x = self.embed_tokens(tokens)

        x += self.embed_positions(
            tokens.view(batch_size * num_alignments, seqlen), positions=res_idx
        ).view(x.size())
...
class MSATransformer1b(nn.Module):
    @classmethod
    def add_args(cls, parser):
        # fmt: off
        parser.add_argument(
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False, res_idx=None
                ):
        if return_contacts:
            need_head_weights = True
