...
from strutils.utils_3d.primitives import Linear, LayerNorm
from strutils.utils_3d.rigid_utils import Rigid

from strutils.utils_3d.tensor_utils import (
    dict_multimap,
    permute_final_dims,
    flatten_final_dims,
)
from strutils.utils_3d.prot_converter import ProtConverter
...
class StructureModuleFullAtom(nn.Module):
    def __init__(
            self,
            c_s,
            c_z,
        ...
        """
        super(StructureModuleFullAtom, self).__init__()
    ...
    def forward(
        ...
        for i in range(n_blocks_act):
            ...
            _bb_update = bb_update(s)
            rigids = rigids.compose_q_update_vec(_bb_update)
            scaled_rigids = rigids.scale_translation(self.trans_scale_factor)

            preds = {
                "scaled_rigids": scaled_rigids,
                "scaled_frames": scaled_rigids.to_tensor_7(),
                "unscaled_frames": rigids.to_tensor_7(),
                "unscaled_rigids": rigids,
                'single': s,
            }

            unnormalized_angles, angles = self.angle_resnet(s, s_initial)
            preds.update({"unnormalized_angles": unnormalized_angles, "angles": angles})
        ...
        def stack(tensor_list):
            if isinstance(tensor_list[0], Rigid):
                return Rigid.cat([t[None] for t in tensor_list], dim=0)
            else:
                return torch.stack(tensor_list)

        outputs = dict_multimap(stack, outputs)
        outputs["cords_ca"] = outputs['scaled_frames'][..., -3:]
        if allatm:
            cord_list = []
            for ii, (frames, angles) in enumerate(zip(outputs['scaled_frames'], outputs['angles'])):
                if return_mid or ii == len(outputs['scaled_frames']) - 1:
                    _frames = Rigid.from_tensor_7(frames)
                    _angles = angles
                    cords, atom_mask, all_frames = self.converter.build_cords(seq, _frames, _angles)
                    cord_list.append([cords, atom_mask, all_frames.to_tensor_4x4()])

            if s.shape[1] > 100:
                torch.cuda.empty_cache()
            outputs['cords_allatm'] = [rearrange(cord[0], 'b l n d->b n l d') for cord in
                                       cord_list]  # [b,14,L, 3]

            outputs["cords_allatm_mask"] = [cord[1].permute(2,1,0) for cord in cord_list]
            outputs["frames_allatm"] = torch.cat([cord[2] for cord in cord_list], dim=0)
